...
add_action('save_post', 'save_custom_meta_frontpage');  
		...
		foreach ($custom_meta_fields_two as $field) {
          ...
          <span id="timestamp"><label style="font-size:13px;" for="'.$field['id'].'">'.$field['label'].'</label><br/><br/>';
					...
					case 'checkbox': 
					    ...
					    echo '<input type="checkbox" name="'.$field['id'].'" id="'.$field['id'].'" ', $meta? ' checked="checked" ' : 'no','/> 
...
// Save the Data
function save_custom_meta_frontpage ($post_id) {
    global $custom_meta_fields_two, $post_id, $post;
	// verify nonce
...
function save_custom_meta_one ($post_id) {
    global $custom_meta_fields, $post_id;
	// verify nonce
	if (!isset($_POST['custom_meta_box_nonce']) || !wp_verify_nonce($_POST['custom_meta_box_nonce'], basename(__FILE__)))
		return $post_id;
	// check autosave
